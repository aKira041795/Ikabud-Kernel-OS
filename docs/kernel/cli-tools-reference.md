...
| `workbench:governance` | Static census of two separate authority axes. Routed operations retain independent `dispatch` (`enforced` / `exempt` / `undeclared`) and handler `reach` (`bus-reachable` / `no-bus-call` / `unresolved`) measures and the routed-only ratio. Non-HTTP capability calls report `transport` (`event`, `workflow`, `cli`, `workbench`, `service`, `worker`) and `scope` (`declared`, `unscoped`, `unresolved`) in a separate count that is never merged into the routed denominator. Dynamic capability/provider routing and unprovable callable paths remain unresolved. `--json` exposes both summaries; `--gate` fails when either frozen routed debt or frozen non-HTTP unscoped debt grows; `--update-baseline` writes both maps in `.governance-baseline.json`. PHP is tokenized, never executed. Source: `kernel/Workbench/Governance/GovernanceCensus.php` |
