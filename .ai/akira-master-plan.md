...
| **P4.3** | Extension trust / capability diff | — | 🔶 **PARTLY DELIVERED** | The **diff half** shipped — see P4.3-diff below. Remaining: the Trust Center's install/update approval and revocation fallback, which cannot be built without P4.1's trust basis (a **director** decision). |
| **P4.3-diff** | Capability diff: what an extension would be granted | `.ai/p4.3-capability-diff.contract.md` | ✅ **COMPLETE** | `php ikabud capability:diff --modules=cms-akira-core,cms-akira-shell` → ADDED 46, **WIDENING 23**, exit 0; widening flags `akira.post.delete@1`, `akira.policy.set_roles@1`, `akira.content_type.delete@1` and **not** the reads; identical across runs; unknown ids named ("the population is incomplete") rather than swallowing a subset; authority rollup unchanged (dispatch 50 / total 50 / undeclared 0 / write_ratio 100); probe 12/0 with a red baseline of 0/1; falsified (revert → RED, restore byte-identical); suite 204/151/53/0 |
...
| 5 | P4.3 extension trust / capability diff | **director** for the remainder. The **diff half is ✅ COMPLETE** 2026-09-20 (P4.3-diff): an operator can now see what an install would grant and which of it writes. What remains is the Trust Center — install/update approval and revocation fallback — and it cannot be built until P4.1 decides what makes an extension admissible (signed / deployment-approved / operator-vetted). |
