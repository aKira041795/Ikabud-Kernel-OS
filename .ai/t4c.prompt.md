...
- Work on the current main working tree. Do NOT git commit, push, or branch. Leave changes uncommitted for chair review.
- Do NOT touch .ai/** or storage/cms-themes/akira-ark-demo/**.
- Rebuild the bundle (npm run type-check && npm run build in modules/cms-akira/cms-akira-builder/admin-ui) so public/admin/assets/cms-akira-builder/** is current.
- There is a pre-existing failing builder test (synthetic-tenant activation). Do not attempt to fix it; just report its status.
