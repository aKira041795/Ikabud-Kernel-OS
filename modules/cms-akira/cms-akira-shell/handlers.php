...
function akiraShellPostForm(array $post = [], string $error = ''): string
{
    ...
    return '<form x-data="{body:' . akiraShellJsString((string)($post['content'] ?? '')) . ', preview:false}" class="grid gap-5 lg:grid-cols-[1fr_280px]" method="post" action="' . akiraShellEscape($action) . '">' . akiraShellCsrfField() . '<div class="space-y-5">' . $errorHtml
