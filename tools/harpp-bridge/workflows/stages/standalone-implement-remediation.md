...
4. **Complete required mock/browser E2E coverage.**
   - Exercise an authenticated PWA against the mock HTTPS bridge through pairing, owner/admin authorization, cursor polling without duplicates, message send/reply, ordered decision lifecycle, illegal transition visibility, revocation, offline drafts, and reconciliation.
   - Inspect browser storage, caches, responses, and logs for credential/token leakage. Current browser tests only inspect DOM/source and do not execute these flows.

5. **Preserve configured polling behavior.**
...
6. **Provide functional decision-detail routing.**
...
if and only if the contract's acceptance criteria AND all six review blockers above are addressed and your verification passes.
