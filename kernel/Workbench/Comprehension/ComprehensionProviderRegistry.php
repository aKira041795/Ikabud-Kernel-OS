...
final class ComprehensionProviderRegistry
{
    ...
    public function register(string $moduleId, callable|string $factory): void
    {
        $this->providers[$moduleId] = $factory;
    }
    public function has(string $moduleId): bool
    {
        return isset($this->providers[$moduleId]) || is_file($this->conventionFile($moduleId));
    }
    public function resolve(string $moduleId): ModuleComprehensionProvider
    {
        ...
        if ($factory === null) {
            throw new \RuntimeException("No comprehension provider for '{$moduleId}'");
        }
        $provider = is_callable($factory) ? $factory() : new $factory();
        if (!$provider instanceof ModuleComprehensionProvider) {
            throw new \UnexpectedValueException("Invalid comprehension provider for '{$moduleId}'");
        }
        return $provider;
    }
    public function modules(): array
    {
        ...
        if (is_dir($modulesRoot)) {
            ...
            foreach ($iterator as $file) {
                if (!$file->isFile() || $file->getFilename() !== 'WorkbenchComprehensionProvider.php') {
                    continue;
                }
