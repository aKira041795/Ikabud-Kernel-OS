...
class ContractComprehensionProvider implements ModuleComprehensionProvider
{
    ...
    public function __construct(string $modulePath)
    {
        ...
        if (!is_array($value)) {
            throw new \RuntimeException("Invalid Workbench contract: {$path}");
        }
        $this->contract = $value;
    }
    ...
    public function routes(): array
    {
        ...
        foreach ((array) ($this->contract['ownership']['routes'] ?? []) as $method => $routes) {
            foreach ((array) $routes as $route) {
                $result[] = ['method' => (string) $method, 'path' => (string) $route, 'handler' => 'module-owned'];
            }
        }
        return $result;
    }
    ...
    {
        return array_values((array) ($this->contract['ownership']['capabilities'] ?? []));
    }
