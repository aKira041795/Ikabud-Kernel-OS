...
final class ComponentScenarioGovernance
{
    ...
    public function validateCatalog(array $catalog): array
    {
        ...
        foreach ($components as $component) {
            if (is_string($component)) {
                $component = ['id' => $component, 'states' => (array) ($catalog['required_states'] ?? []), 'variants' => (array) ($catalog['required_variants'] ?? [])];
            }
            $id = (string) ($component['id'] ?? 'unknown');
            $states = (array) ($component['states'] ?? []);
            foreach (self::REQUIRED_STATES as $state) {
                if (!in_array($state, $states, true)) {
                    $errors[] = "{$id}: missing state {$state}";
                }
            }
            ...
            foreach (self::REQUIRED_VIEWPORTS as $viewport) {
                if (!in_array($viewport, $viewports, true)) {
                    $errors[] = "{$id}: missing viewport {$viewport}";
                }
            }
            foreach (['themes', 'locales', 'data_shapes'] as $variant) {
                if (($component['variants'][$variant] ?? []) === []) {
                    $errors[] = "{$id}: missing {$variant} variants";
                }
            }
        }
        if ($components === []) {
            $errors[] = 'component catalog is empty';
        }
    ...
    public function approve(array $comparison, string $approver, string $reason): array
    {
        if (!($comparison['changed'] ?? false)) {
            throw new \RuntimeException('No visual baseline change to approve');
        }
        if (trim($approver) === '' || trim($reason) === '') {
            throw new \RuntimeException('Approver and reason are required');
        }
    ...
    public function affectedModules(string $component, array $contracts): array
    {
        ...
        foreach ($contracts as $contract) {
            foreach ((array) ($contract['pages'] ?? []) as $page) {
                if (in_array($component, (array) ($page['required_components'] ?? []), true)) {
                    $modules[] = (string) ($contract['module']['id'] ?? '');
                }
            }
        }
