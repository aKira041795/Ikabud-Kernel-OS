...
final class WeightedPathPlanner
{
        ...
        for ($round = 1; $round < max(1, $k); $round++) {
            ...
            for ($i = 0; $i < count($previous['nodes']) - 1; $i++) {
                ...
                foreach ($accepted as $path) {
                    if (array_slice($path['nodes'], 0, $i + 1) === $rootNodes && isset($path['edges'][$i])) {
                        $blocked[$path['edges'][$i]] = true;
                    }
                }
            ...
            $nextKey = array_key_first($candidates);
            $accepted[] = $candidates[$nextKey];
            unset($candidates[$nextKey]);
        }
        return $accepted;
    }
    ...
    public function selectSuite(array $paths, float $budget, array $mandatoryNodes = []): array
    {
        ...
        foreach ($paths as $path) {
            if (array_intersect($mandatoryNodes, $path['nodes'] ?? []) === []) {
                continue;
            }
            if ($spent + $path['cost'] <= $budget) {
                $selected[] = $path;
                $spent += $path['cost'];
        ...
        while ($remaining !== []) {
            ...
            if ($spent + $next['cost'] > $budget) {
                continue;
            }
            $selected[] = $next;
            $spent += $next['cost'];
            foreach ($next['edges'] as $e) {
                $covered[$e] = true;
            }
        }
        return $selected;
    ...
    {
        if ($graph->node($start) === null || $graph->node($target) === null) {
            return null;
        }
        $dist = [$start => 0.0];
        $previousNode = [];
        ...
        while ($queue !== []) {
            ...
            $nodeDistance = $queue[$node];
            unset($queue[$node]);
            if ($node === $target) {
                break;
            }
            foreach ($graph->node($node)->edgesOut as $edgeKey) {
                if (isset($blockedEdges[$edgeKey])) {
                    continue;
                }
                $edge = $graph->edges()[$edgeKey] ?? null;
                if ($edge === null || isset($blockedNodes[$edge->to])) {
                    continue;
                }
                $edgeWeight = max(0.0, (float)$weight($edgeKey));
                $candidate = $nodeDistance + $edgeWeight;
                if ($candidate < ($dist[$edge->to] ?? INF)) {
                    $dist[$edge->to] = $candidate;
                    $previousNode[$edge->to] = $node;
                    $previousEdge[$edge->to] = $edgeKey;
                    $queue[$edge->to] = $candidate;
                }
            }
        }
        if (!isset($dist[$target])) {
            return null;
        }
        $nodes = [$target];
        $edges = [];
    ...
    {
        $risk = $this->unit($s['risk'] ?? 0);
        $novelty = $this->unit($s['novelty'] ?? 0);
        $gap = $this->unit($s['gap'] ?? 0);
        $impact = $this->unit($s['impact'] ?? 0);
        $uncertainty = $this->unit($s['uncertainty'] ?? 0);
    ...
    private function pathValue(array $edges, array $stats): float
    {
        $v = 0.0;
        foreach ($edges as $e) {
            $s = $stats[$e] ?? [];
            $v += $this->unit($s['risk'] ?? 0) + $this->unit($s['novelty'] ?? 0) + $this->unit($s['gap'] ?? 0) + $this->unit($s['impact'] ?? 0) + $this->unit($s['uncertainty'] ?? 0);
        } return round($v, 4);
    }
    private function pathCost(array $edges, callable $weight): float
    {
        $sum = 0.0;
        foreach ($edges as $e) {
            $sum += max(0.0, (float)$weight($e));
        } return round($sum, 6);
    }
    private function unit(mixed $v): float
    {
