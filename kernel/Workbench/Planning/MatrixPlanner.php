...
final class MatrixPlanner
{
    ...
    public function plan(array $dimensions, array $mandatory = [], int $budget = 24): array
    {
        ...
        foreach ($all as $combination) {
            if ($this->matchesAny($combination, $mandatory)) {
                $selected[$this->key($combination)] = $combination;
            }
        }
        $pairUniverse = [];
        foreach ($all as $combination) {
            foreach ($this->pairs($combination) as $pair) {
                $pairUniverse[$pair] = true;
            }
        }
        $covered = [];
        foreach ($selected as $combination) {
            foreach ($this->pairs($combination) as $pair) {
                $covered[$pair] = true;
            }
        }
        while (count($selected) < min($budget, count($all)) && count($covered) < count($pairUniverse)) {
            ...
            foreach ($this->pairs($best) as $pair) {
                $covered[$pair] = true;
            }
        }
        ksort($selected);
        ...
        foreach ($all as $combination) {
            $key = $this->key($combination);
            if (!isset($selected[$key])) {
                $omitted[] = ['combination' => $combination, 'reason' => count($selected) >= $budget ? 'risk budget reached after mandatory and pairwise coverage' : 'adds no uncovered dimension pair'];
            }
        }
    ...
    public function detectIsolationLeaks(array $observations): array
    {
        ...
        foreach ($observations as $observation) {
            ...
            if ($active !== '' && $observed !== '' && $active !== $observed) {
                $leaks[] = ['severity' => 'critical', 'category' => 'tenant-isolation', 'observation' => $observation];
            }
        }
        return $leaks;
    }
    ...
    private function matchesAny(array $combination, array $rules): bool
    {
        foreach ($rules as $rule) {
            if (array_diff_assoc($rule, $combination) === []) {
                return true;
            }
        }
        return false;
    ...
    private function pairs(array $combination): array
    {
        ...
        for ($i = 0; $i < count($keys); $i++) {
            for ($j = $i + 1; $j < count($keys); $j++) {
                $pairs[] = $keys[$i] . '=' . $combination[$keys[$i]] . '|' . $keys[$j] . '=' . $combination[$keys[$j]];
            }
        }
        return $pairs;
    }
    ...
    {
        return implode('|', array_map(static fn ($k, $v): string => "{$k}={$v}", array_keys($combination), $combination));
    }
}
