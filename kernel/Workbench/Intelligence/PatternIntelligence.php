...
final class FinalEvidenceAssembler
{
    public function assemble(array $sources): array
    {
        ...
        foreach ($sources as $source => $packet) {
            ...
            foreach (($packet['successful_checks'] ?? []) as $check) {
                $successful[] = $check;
            }
        }
        $correlated = [];
        foreach ($observations as $item) {
            ...
            if (!isset($correlated[$key])) {
                $correlated[$key] = $item + ['evidence' => [], 'occurrences' => 0, 'fingerprint' => $key];
            }
    ...
    private function stableJson(mixed $value): string
    {
        if (is_array($value)) {
            ...
            foreach ($value as $k => $v) {
                $value[$k] = json_decode($this->stableJson($v), true);
            }
        }
...
final class ClaimContract
{
    ...
    public function validate(array $assessment, array $knownEvidenceIds = [], array $knownRoutes = []): array
    {
        ...
        foreach (($assessment['claims'] ?? []) as $i => $claim) {
            if (!in_array($claim['claim_type'] ?? '', self::TYPES, true)) {
                $errors[] = "claims.{$i}.claim_type";
            }
            if (trim((string)($claim['text'] ?? '')) === '') {
                $errors[] = "claims.{$i}.text";
            }
            if (($claim['claim_type'] ?? '') !== 'unknown' && empty($claim['evidence_ids']) && empty($claim['source_ids'])) {
                $errors[] = "claims.{$i}.support";
            }
            foreach (($claim['evidence_ids'] ?? []) as $id) {
                if ($knownEvidenceIds !== [] && !in_array($id, $knownEvidenceIds, true)) {
                    $errors[] = "claims.{$i}.invalid_evidence:{$id}";
                }
            }
        }
        foreach (($assessment['assumptions'] ?? []) as $i => $assumption) {
            if (!in_array($assumption['verification_status'] ?? 'proposed', self::STATES, true)) {
                $errors[] = "assumptions.{$i}.verification_status";
            }
            foreach (($assumption['routes'] ?? []) as $route) {
                if ($knownRoutes !== [] && !in_array($route, $knownRoutes, true)) {
                    $errors[] = "assumptions.{$i}.invented_route:{$route}";
                }
            }
        }
...
final class GovernedRetriever
{
    ...
    {
        if ((int)($this->policy['authority_level'] ?? 0) < 3) {
            throw new RuntimeException('External retrieval requires authority level 3');
        }
        $host = strtolower((string)parse_url($url, PHP_URL_HOST));
        if (!in_array($host, array_map('strtolower', (array)($this->policy['external_hosts'] ?? [])), true)) {
            throw new RuntimeException('External host not approved');
        }
        if (!is_callable($this->externalFetcher)) {
            throw new RuntimeException('External fetcher unavailable');
        }
...
final class GrainSignature
{
    ...
    private function counts(array $rows, string $key): array
    {
        $out = [];
        foreach ($rows as $r) {
            $v = (string)($r[$key] ?? 'unknown');
            $out[$v] = ($out[$v] ?? 0) + 1;
        } ksort($out);
        return $out;
    }
    private function stable(array $v): string
    {
    ...
    private function numericLeaves(array $v, string $prefix = ''): array
    {
        $out = [];
        foreach ($v as $k => $x) {
            $p = $prefix === '' ? (string)$k : "{$prefix}.{$k}";
            if (is_array($x)) {
                $out += $this->numericLeaves($x, $p);
            } elseif (is_numeric($x)) {
                $out[$p] = (float)$x;
            }
        }return $out;
...
final class VerifiedCaseRetriever
{
    public function retrieve(array $cases, array $scope, array $signature, int $limit = 5): array
    {
        ...
        foreach ($cases as $case) {
            if (!($case['verified'] ?? false)) {
                continue;
            }
            if (($case['module'] ?? '') !== ($scope['module'] ?? '')) {
                continue;
            }
            if (!empty($scope['component']) && !empty($case['component']) && $case['component'] !== $scope['component']) {
                continue;
            }
            if (!empty($case['expires_at']) && strtotime((string)$case['expires_at']) < time()) {
                continue;
            }
...
final class TargetedTestValidator
{
    public function validate(array $plan, array $graph, int $authorityLevel): array
    {
        ...
        foreach (['hypothesis', 'preconditions', 'actions', 'expected_observations', 'cleanup', 'information_gain'] as $key) {
            if (!isset($plan[$key])) {
                $errors[] = $key;
            }
        }
        ...
        foreach (($plan['actions'] ?? []) as $action) {
            if (!isset($nodes[$action])) {
                $errors[] = "unknown_action:{$action}";
            }
        }
        if ((float)($plan['information_gain'] ?? -1) < 0 || (float)($plan['information_gain'] ?? 2) > 1) {
            $errors[] = 'information_gain_range';
        }
...
final class GoldenEvaluator
{
    ...
    public function evaluate(array $cases, callable $runner): array
    {
        ...
        foreach ($cases as $case) {
            ...
            if (($r['prediction'] ?? '') === ($case['expected'] ?? '')) {
                $top1++;
            }if (($r['prediction'] ?? '') === 'unknown' && ($case['expected'] ?? '') === 'unknown') {
                $abstain++;
            }$unsupported += count($r['unsupported_claims'] ?? []);
        }
...
final class ChangeRecommendationGate
{
    ...
    {
        $errors = [];
        if ((int)($context['authority_level'] ?? 0) < 6) {
            $errors[] = 'authority_level';
        }
        if (!($context['human_approved'] ?? false)) {
            $errors[] = 'human_approval';
        }
        if (empty($recommendation['evidence_ids'])) {
            $errors[] = 'evidence_ids';
        }
        if (!isset($recommendation['expected_telemetry'],$recommendation['verification'],$recommendation['rollback'])) {
            $errors[] = 'verification_contract';
        }
        if (($recommendation['updates_baseline'] ?? false) === true) {
            $errors[] = 'baseline_update_forbidden';
        }
...
final class PatternIntelligenceEngine
{
    public function analyze(array $sources, array $context): array
    {
        ...
        if (isset($context['ai_assessment'])) {
            $result['ai_assessment'] = $context['ai_assessment'];
        }
        if (isset($context['effective_ai_policy'])) {
            $result['effective_ai_policy'] = $context['effective_ai_policy'];
        }
        return $result;
    }
}
