...
final class RunIntelligence
{
    ...
    {
        usort($events, static fn (array $a, array $b): int => [(string) ($a['at'] ?? ''), (int) ($a['sequence'] ?? 0)] <=> [(string) ($b['at'] ?? ''), (int) ($b['sequence'] ?? 0)]);
        return $events;
    }
    ...
    {
        if ($fingerprint === '' || $owner === '' || $reason === '' || new \DateTimeImmutable($expiresAt) <= new \DateTimeImmutable()) {
            throw new \RuntimeException('Quarantine requires fingerprint, owner, reason, and future expiry');
        }
