...
final class WorkbenchMetrics
{
    ...
    public function record(string $metric, array $labels = [], float $value = 1.0): void
    {
        ...
        try {
            ...
            if (!is_array($data)) {
                $data = [];
            } $key = $metric.'|'.hash('sha256', json_encode($labels));
            $data[$key] ??= ['metric' => $metric,'labels' => $labels,'count' => 0,'sum' => 0.0,'last_seen' => null];
            $data[$key]['count']++;
            $data[$key]['sum'] += $value;
            $data[$key]['last_seen'] = date('c');
    ...
    {
        $v = is_file($this->file) ? json_decode((string)file_get_contents($this->file), true) : [];
